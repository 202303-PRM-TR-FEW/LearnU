...
async function SearchForm() {
  ...
  const checkboxes = options.map((option, index) => (
    <label htmlFor={`checkboxLabel${index}`} className="  font-semibold text-slate-700 dark:text-white undefined item max-w-10 border border-transparent flex cursor-pointer select-none items-center mb-3 sm:mb-6 mr-7" key={index}>
  ...
  return (
    <div className=''>
      ...
      <hr className="w-11/12 mb-3"/>
        ...
        <hr className="w-11/12 mb-3"/>
        <span className={classNames.categoryContainerText}>RECOMMANDED</span>
        <div className='md:flex md:flex-wrap grid grid-cols-1'>
        {courses.map((course) => (
                    <Recommanded key={course.id} title={course.courseName} img={course.img} mins={course.duration.mins} hours={course.duration.hours} rating={course.rating} price={course.price} trainer={course.trainerName} />
                ))}
        </div>
    </div>
  );
